...
@endphp
<div class="bg-white border rounded-sm shadow-lg col-span-full sm:col-span-6 xl:col-span-4 border-slate-200">
    <div class="flex flex-col h-full p-5">
        <header>
            <div class="flex items-center justify-between">
                @if ($campaign->category === '1')
                    <div class="flex items-center justify-center w-10 h-10 rounded-full shrink-0 bg-rose-500">
                        <svg class="fill-current w-9 h-9 text-rose-50" viewBox="0 0 36 36">
                            <path d="M25 24H11a1 1 0 01-1-1v-5h2v4h12v-4h2v5a1 1 0 01-1 1zM14 13h8v2h-8z" />
                        </svg>
                    </div>
                @elseif ($campaign->category === '2')
                    <div class="flex items-center justify-center w-10 h-10 rounded-full shrink-0 bg-emerald-500">
                        <svg class="fill-current w-9 h-9 text-emerald-50" viewBox="0 0 36 36">
                            <path d="M15 13v-3l-5 4 5 4v-3h8a1 1 0 000-2h-8zM21 21h-8a1 1 0 000 2h8v3l5-4-5-4v3z" />
                        </svg>
                    </div>
                @elseif ($campaign->category === '3')
                    <div class="flex items-center justify-center w-10 h-10 rounded-full shrink-0 bg-sky-500">
                        <svg class="fill-current w-9 h-9 text-sky-50" viewBox="0 0 36 36">
                            <path
                ...
                @elseif ($campaign->category === '4')
                    <div class="flex items-center justify-center w-10 h-10 bg-indigo-500 rounded-full shrink-0">
                ...
                @endif
                <div class="flex -ml-px -space-x-3 shrink-0">
                    @foreach ($campaign->members as $member)
                        <a class="block" href="{{ $member->link }}">
                            <img class="box-content border-2 border-white rounded-full"
                                src="{{ asset('images/' . $member->image) }}" width="28" height="28"
                                alt="{{ $member->name }}" />
                        </a>
                    @endforeach
                </div>
        ...
        <div class="mt-2 grow">
            <a class="inline-flex mb-1 text-slate-800 hover:text-slate-900" href="#0">
                <h2 class="text-xl font-semibold leading-snug">{{ $campaign->title }}</h2>
            </a>
        ...
        <footer class="mt-5">
            ...
            <div class="flex items-center justify-between">
                <div>
                    <div
                        class="text-xs inline-flex font-medium rounded-full text-center px-2.5 py-1 {{ $status_color }}">
                        {{ $campaign->type }}</div>
                </div>
                <div>
